...
## Run Parameter Audit & Tracking

To ensure reproducibility, every cohort analysis automatically documents its execution parameters in the target results directory:

* `RUN_PARAMETERS.md`: Human-readable Markdown summary documenting execution date, host system, input/output paths, CLI parameters, software configuration, and sample list.
* `run_parameters.json`: Machine-readable JSON metadata file.
* `mapeo_muestras_parametros.csv`: Mapping table generated by `update_mapping.py` linking sample IDs, run types, fusion callers, rRNA filtering options, and parameter strings.

To generate or update these parameter logs manually after a run:
```bash
python3 nextflow_pipeline/bin/document_run_params.py \
  --outdir /path/to/results \
